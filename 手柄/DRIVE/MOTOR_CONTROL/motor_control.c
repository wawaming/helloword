...
uint8_t Servo_Set_Function(Servo_Struct *Servo_Structure)
{
	...
	while(!(CAN_TransmitStatus(CAN1, msgbox)==CAN_TxStatus_Ok)){BEED_ON;i++;delay_us(20);if(i>500){return 1;BEED_OFF;}}
	/******消息发送成功*****/
	BEED_OFF;
	return 0;
}
...
/**
	...
	*/
uint8_t Motor_MastSpeed(SERVO_MLCB *Servo_MLCB,float data)
{
	uint8_t error;
	Servo_Struct Servo_Structure;
	Servo_Structure.Data_Format = CAN_Id_Standard;					//标准帧格式
...
/**
	...
	*	@retval	error	:	0	数据发送成功,		其他	数据发送失败
	*/
uint8_t Motor_AccelSpeed(SERVO_MLCB *Servo_MLCB,float data)
{
	uint8_t error;
	Servo_Struct Servo_Structure;
	Servo_Structure.Data_Format = CAN_Id_Standard;					//标准帧格式
...
/**
	...
	*	@retval	error	0：数据发送正常		其他：数据发送失败
	*/
uint8_t Motor_ToRelativePlace(SERVO_MLCB *Servo_MLCB,int data)
{
	uint8_t error;
	Servo_Struct Servo_Structure;
	Servo_Structure.Data_Format = CAN_Id_Standard;					//标准帧格式
	...
	//Servo_Structure.Last_Data = (data*(Servo_MLCB->Ratio)*(4*(Servo_MLCB->Coder))/360.0f);
	Servo_Structure.Last_Data=data;
	error = Servo_Set_Function(&Servo_Structure);
	return error;
}
...
/**
	...
	*	@retval	error	0：发送数据成功	其他：数据发送失败
	*/
uint8_t Motor_PeakCurrent_Set(SERVO_MLCB *Servo_MLCB,int32_t data)
{
	uint8_t error;
	Servo_Struct Servo_Structure;
	Servo_Structure.Data_Format = CAN_Id_Standard;					//标准帧格式
	Servo_Structure.Data_Type   = CAN_RTR_Data;							//数据帧
	Servo_Structure.Servo_ID    = Servo_MLCB->Star_Id;
	Servo_Structure.Data_Len		= ML_SPC_LEN;
	
	Servo_Structure.Front_Data[0] = ML_SPC_LEN;
	Servo_Structure.Front_Data[1] = Servo_MLCB->Device_Id;
	Servo_Structure.Front_Data[2] = ML_SPC;
	Servo_Structure.Front_Data[3] = 0;
...
/**
	...
	*	@retval	error	0：发送数据成功	其他：数据发送失败
	*/
uint8_t Motor_ContinuousCurrent_Set(SERVO_MLCB *Servo_MLCB,int32_t data)
{
	uint8_t error;
	Servo_Struct Servo_Structure;
	Servo_Structure.Data_Format = CAN_Id_Standard;					//标准帧格式
	Servo_Structure.Data_Type   = CAN_RTR_Data;							//数据帧
	Servo_Structure.Servo_ID    = Servo_MLCB->Star_Id;
	Servo_Structure.Data_Len		= ML_SCC_LEN;
	
	Servo_Structure.Front_Data[0] = ML_SCC_LEN;
	Servo_Structure.Front_Data[1] = Servo_MLCB->Device_Id;
	Servo_Structure.Front_Data[2] = ML_SCC;
	Servo_Structure.Front_Data[3] = 0;
...
/**
	*	@brief	电机向绝对位置运行 位置运动相对于设置的绝对位置
	*	@param	*Servo_MLCB		需要设置的控制块
	*	@retval	error	0：发送数据成功	其他：数据发送失败
	*/
...
uint8_t Motor_SBS(SERVO_MLCB *Servo_MLCB)
{
	...
	Servo_Structure.Data_Type   = CAN_RTR_Data;							//数据帧
	Servo_Structure.Servo_ID    = Servo_MLCB->Star_Id;
	Servo_Structure.Data_Len		= ML_SBS_LEN;
	
	Servo_Structure.Front_Data[0] = ML_SBS_LEN;
	Servo_Structure.Front_Data[1] = Servo_MLCB->Device_Id;
	Servo_Structure.Front_Data[2] = ML_SBS;
	Servo_Structure.Front_Data[3] = 0;
...
uint8_t Motor_CBS(SERVO_MLCB *Servo_MLCB)
{
	...
	Servo_Structure.Data_Type   = CAN_RTR_Data;							//数据帧
	Servo_Structure.Servo_ID    = Servo_MLCB->Star_Id;
	Servo_Structure.Data_Len		= ML_CBS_LEN;
	
	Servo_Structure.Front_Data[0] = ML_CBS_LEN;
	Servo_Structure.Front_Data[1] = Servo_MLCB->Device_Id;
	Servo_Structure.Front_Data[2] = ML_CBS;
	Servo_Structure.Front_Data[3] = 0;
