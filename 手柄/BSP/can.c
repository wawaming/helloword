...
void CAN_Config(uint8_t CAN_MODE)
{
	...
	CAN_FilterInitStructure.CAN_FilterIdHigh = CAN_Filter0_IDHigh<<5;			//stdid<<5放入//标识符的高16位ID(0x0000~0xffff),即配置CAN_FxR0寄存器
	CAN_FilterInitStructure.CAN_FilterIdLow = 0|CAN_ID_STD;	             //标识符的低16位ID(0x0000~0xffff),即配置CAN_FxR0寄存器
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh =CAN_Filter0_IDLow<<5; 	//掩码的高16位ID(0x0000~0xffff),即配置CAN_FxR1寄存器
	CAN_FilterInitStructure.CAN_FilterMaskIdLow =0|CAN_ID_STD; 		       //掩码的低16位ID(0x0000~0xffff),即配置CAN_FxR1寄存器
...
uint8_t Send_underpan(uint8_t data)
{
	int i;
	uint8_t msgbox;
	CanTxMsg	TxMessage;
	...
	TxMessage.IDE		= CAN_Id_Standard;				//标准帧格式
	TxMessage.RTR		= CAN_RTR_Data;						//数据帧
	TxMessage.StdId = 0x07;										//标准ID
	TxMessage.ExtId = 0;										  //扩展ID
	TxMessage.DLC 	= 8;										//传送的数据长度,字节数
	TxMessage.Data[0]	= data;								//传输的数据
